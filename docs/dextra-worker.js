  ...
  var DataWorker = class extends DextraWorker {
    ...
    initDb(db) {
      return __async(this, null, function* () {
        console.log("THIS SHOULD BE DUCKDB PLEASE", db);
        if (!this.db) {
          return true;
        } else {
          return true;
    ...
    query(sql) {
      return __async(this, null, function* () {
        const db = this.db;
        const c = yield db.connect();
        const result = yield c.query(sql);
        c.close();
        return result;
      });
    }
    loadData(dataSchema) {
      return __async(this, null, function* () {
        ...
        switch (dataSchema.type) {
          case "csv":
            const text = yield fetch(dataSchema.url).then((res) => res.text());
            yield db.registerFileText(dataSchema.url, text);
            const columns = yield this.query(`SELECT * FROM "${dataSchema.url}" LIMIT 1`);
            if (!columns) {
    ...
    runAnalysis(dataSchema, columnSchema, analysisSchema) {
      return __async(this, null, function* () {
        const { state, columns } = this.data[dataSchema.url];
        if (state !== "loaded" || columns.indexOf(columnSchema.column) === -1) {
          return null;
        }
        const result = yield this.query(`SELECT "${columnSchema.column}" FROM "${dataSchema.url}"`);
        const columnData = result.toArray().map((f) => f[columnSchema.column]);
        return simple_statistics_exports[analysisSchema.type](columnData);
      });
    }
    getTable(dataSchema) {
      return __async(this, null, function* () {
        const data = yield this.query(`SELECT * FROM "${dataSchema.url}"`);
        return data.toArray();
      });
    }
  };
